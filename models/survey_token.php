...
class SurveyToken extends AFWObject {
    ...
    public function additional($field_name, $col_struct)
        {
                ...
                if ($col_struct == "obsolete") return false;
                if ($col_struct == "required") return (!$params["optional"]);

                if ($col_struct == "function_col_name") return 'stars';
                if ($col_struct == "stars") return 5;
                if ($col_struct == "format") 
                {
                    if(AfwStringHelper::stringStartsWith($field_name, "attribute_enum_")) return "STARS";
                    if(AfwStringHelper::stringStartsWith($field_name, "attribute_yn_")) return "ICON";
                    
                    
                }
                if ($col_struct == "checkbox") return false;
